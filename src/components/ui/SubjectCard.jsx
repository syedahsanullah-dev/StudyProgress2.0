      ...
      >
        ...
        <div className="flex items-start justify-between mb-4 gap-3" style={{ transform: "translateZ(30px)" }}>
          ...
          <div className="flex flex-col items-end gap-1 shrink-0">
            {grade && grade !== 'N/A' && grade !== 'P' ? (
              <div className="flex items-center gap-1.5">
                <span className={`px-2 py-0.5 rounded-md border font-extrabold text-xs shadow-sm ${getGradeStyle(grade)}`}>
                  {grade}
                </span>
                <div className={`px-2.5 py-0.5 rounded-md border font-extrabold text-xs shadow-sm bg-slate-800 text-emerald-400 border-emerald-500/30`}>
                  {calculatedGPA !== undefined ? `${calculatedGPA.toFixed(2)} GPA` : 'N/A'}
                </div>
              </div>
            ) : grade === 'P' ? (
              <div className="px-2.5 py-1 rounded-lg border font-extrabold text-xs shadow-sm bg-blue-500/15 text-blue-300 border-blue-500/30">
                Pass (Non-Graded)
              </div>
            ) : (
              <div className={`px-3 py-1 rounded-lg border font-extrabold text-sm shadow-sm bg-slate-800 text-emerald-400 border-emerald-500/30`}>
                {calculatedGPA !== undefined && calculatedGPA > 0 ? `${calculatedGPA.toFixed(2)} GPA` : 'N/A'}
              </div>
            )}
