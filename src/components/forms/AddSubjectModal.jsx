...
import { X, BookOpen, Terminal, Loader2, Target, Layers } from 'lucide-react';
import { db, auth } from '../../../firebase';
import { collection, addDoc, serverTimestamp } from 'firebase/firestore';
import useStore from '../../store/useStore';

export default function AddSubjectModal({ isOpen, onClose }) {
  const { semesters, activeSemesterId } = useStore();
  ...
  useEffect(() => {
    if (isOpen) {
      // Default to active semester if valid, or first semester
      const initialSemId = (activeSemesterId && activeSemesterId !== 'all') 
        ? activeSemesterId 
        : (semesters[0]?.id || '');
      setSemesterId(initialSemId);
    }
  }, [isOpen, activeSemesterId, semesters]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const targetSemId = semesterId || (semesters[0]?.id || null);
            ...
            {semesters.length > 0 && (
              <div>
                <label className="block text-sm font-semibold text-slate-300 mb-2 flex items-center gap-2">
                  ...
                  <span>Assign to Semester / Term</span>
                </label>
                <select
                  value={semesterId}
                  onChange={(e) => setSemesterId(e.target.value)}
                  className="w-full bg-slate-900/50 border border-slate-700 text-slate-100 rounded-xl py-3 px-4 focus:outline-none focus:border-indigo-500 transition-all text-sm"
                >
                  {semesters.map((sem) => (
