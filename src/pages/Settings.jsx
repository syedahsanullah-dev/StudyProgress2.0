...
import { User, Monitor, AlertTriangle, Save, Loader2, Check, DownloadCloud, Database, X, RefreshCw, Layers, Settings2 } from 'lucide-react';
import useStore from '../store/useStore';

export default function Settings() {
  const { semesters, activeSemesterId, downloadAllData } = useStore();
  ...
  const [showSemestersModal, setShowSemestersModal] = useState(false);
  ...
  const confirmFactoryReset = async () => {
    ...
    try {
      ...
      // Fetch all semesters for this user
      const qSemesters = query(collection(db, 'semesters'), where('userId', '==', uid));
      const semestersSnap = await getDocs(qSemesters);
      semestersSnap.forEach(docSnap => batch.delete(docSnap.ref));

      // Fetch all datesheet items for this user
      const qDatesheet = query(collection(db, 'datesheet'), where('userId', '==', uid));
      const datesheetSnap = await getDocs(qDatesheet);
      datesheetSnap.forEach(docSnap => batch.delete(docSnap.ref));
  ...
  const confirmExportData = async () => {
    ...
    try {
      ...
      alert("Failed to export all Firestore data.");
    } finally {
      setIsExporting(false);
    }
  };

  const activeSem = semesters.find(s => s.id === activeSemesterId) || semesters.find(s => s.isCurrent) || semesters[0];
  ...
  return (
    <div className="min-h-screen bg-transparent flex">
      ...
      <div className="flex-1 flex flex-col sm:ml-20 pb-20 sm:pb-0 min-h-screen">
        ...
        <div className="p-4 sm:p-8 max-w-4xl mx-auto w-full space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div>
            ...
            <p className="text-slate-400 mt-1">Manage your account preferences, semesters, and app behavior.</p>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-xl">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-xl font-bold text-white">Academic Semesters</h2>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {semesters.length} {semesters.length === 1 ? 'semester' : 'semesters'} configured • Active: {activeSem?.name || 'All'}
                    </p>
                  </div>
                </div>

                <button
                  onClick={() => setShowSemestersModal(true)}
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl text-xs sm:text-sm transition-all shadow-lg shadow-indigo-600/20 flex items-center gap-2"
                >
                  <Settings2 size={16} />
                ...
                {semesters.map(sem => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="font-bold text-sm">{sem.name}</span>
                      {sem.isCurrent && (
                    ...
                    <div className="text-xs text-slate-400 mt-1 flex items-center justify-between">
                      <span>Target: {Number(sem.targetSGPA || 3.5).toFixed(2)} GPA</span>
                      <span>Order: #{sem.order || 1}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-3xl p-6 shadow-xl">
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-xl font-bold text-white">Data Management & Backup</h2>
              </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-slate-800/30 rounded-2xl border border-slate-700/30 gap-4">
                <div>
                  <h3 className="text-white font-semibold">Complete Firestore Backup</h3>
                  <p className="text-sm text-slate-400 mt-1">
                    Download a full snapshot of all collections: semesters, subjects, assessments, notes, and datesheets directly from Firestore.
                  </p>
                </div>
            ...
            <div className="bg-red-500/5 border border-red-500/20 rounded-3xl p-6 shadow-xl">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-red-500/5 rounded-2xl border border-red-500/10 gap-4">
                <div>
                  ...
                  <p className="text-sm text-red-400/80 mt-1">Permanently delete all your semesters, subjects, assessments, datesheets, and notes. This cannot be undone.</p>
                </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0F172A] border border-slate-700/50 w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-300 text-sm mb-6">
              This will query live Firestore collections (<strong className="text-emerald-400">semesters, subjects, assessments, datesheet</strong>) and download a complete JSON backup file with timestamps.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                ...
                disabled={isExporting}
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-medium rounded-xl transition-all flex items-center gap-2 disabled:opacity-50"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-[#0F172A] border border-red-500/30 w-full max-w-md rounded-2xl p-6 shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <p className="text-slate-300 text-sm mb-4">
              This will <strong className="text-red-400">permanently delete ALL your data</strong>, including all semesters, subjects, grades, datesheets, and notes. This action cannot be undone.
            </p>
            <div className="mb-6">
      ...
      {/* Semesters Management Modal */}
      <ManageSemestersModal 
        isOpen={showSemestersModal}
        onClose={() => setShowSemestersModal(false)}
      />
